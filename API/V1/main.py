...
class Goaloo:
    ...
    def set_games(self) -> None:
        th: Thread = Thread(target=self.get_games, args=(self.home_results, 'table_v1',))
        th2: Thread = Thread(target=self.get_games, args=(self.away_results, 'table_v2',))
        th.start();th2.start()
        th.join();th2.join()
        self.datas['results'] = {'Home': self.home_results, 'Away': self.away_results}
    
    def start(self) -> None:
        th: Thread =Thread(target=self.get_time_league)
        th2: Thread =Thread(target=self.get_goals)
        th3: Thread =Thread(target=self.get_odds)
        th4: Thread = Thread(target=self.set_games)
        th.start();th2.start();th3.start();th4.start()
        th.join();th2.join();th3.join();th4.join()
